...
class AdminBukuDiterimaController extends \crocodicstudio\crudbooster\controllers\CBController {

    public function __construct() {
        $this->table              = "buku_diterima";
        $this->primary_key        = "id";
		...
		$this->col[] = array("label"=>"No Donasi","name"=>"id_donasi_item","callback_php"=>'$this->code("[id_donasi_item]")');
		$this->col[] = array("label"=>"Judul","name"=>"judul" );
		$this->col[] = array("label"=>"Penulis","name"=>"penulis" );
		$this->col[] = array("label"=>"Penerbit","name"=>"penerbit" );

		$this->form = array();
		$this->form[] = array("label"=>"No Donasi","name"=>"id_donasi_item","type"=>"select","required"=>TRUE,"validation"=>"required|integer|min:0","datatable"=>"donasi_item,id_donasi");
		$this->form[] = array("label"=>"Judul","name"=>"judul","type"=>"text","required"=>TRUE,"validation"=>"required|min:3|max:255");
		$this->form[] = array("label"=>"Penulis","name"=>"penulis","type"=>"text","required"=>TRUE,"validation"=>"required|min:3|max:255");
    ...
    */
    ...
    public function Code($id){
        $check = DB::table('donasi_item')->where('id',$id)->first();

        $get_code = DB::table('donasi')->where('id',$check->id_donasi)->first();

        return $get_code->no_donasi;
    }
